...
public class GameController : MonoBehaviour
{
   ...
   private bool IsLose, firstCube = false;
    ...
    private UnityEngine.Color toCameraColor;
...
private int prevCountMaxHorizontal = 0;
   ...
   private void Update()
   {
    if((Input.GetMouseButtonDown(0) || Input.touchCount > 0 || Input.anyKeyDown) &&  cubeToPlace != null && !EventSystem.current.IsPointerOverGameObject()){
        #if !UNITY_EDITOR
        if(InversePropertyAttribute.GetTouch(0).phase != TouchPhase.Began)
        return;
        #endif 
     ...
     if( !IsLose && allCubesRB.velocity.magnitude > 0.1f){
       ...
       Camera.main.backgroundColor = UnityEngine.Color.Lerp(Camera.main.backgroundColor, toCameraColor, Time.deltaTime / 1.5f);
   }
